...
1. `.orc/state.json` — active orc:dispatch run (wave, task counts, agent status)
...
## --project <slug> (unified view)

When the user asks for a project-scoped view (e.g. `/orc:status --project talos`), run:

```bash
bash "$(dirname "$0")/scripts/status-project.sh" <project_slug>
```

Then render the returned markdown verbatim.
